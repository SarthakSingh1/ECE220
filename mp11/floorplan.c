#include "floorplan.h"
/* This program uses binary trees to organize rectangles to make one bigger rectangle. The tree holds module nodes, which hold information that defines
how the final shape will be cut. We begin by creating the tree using recursion (init_slicing_tree) (initializing variables for the node (pointers)) 
after parsing information from a file. Many of the functions afterwards, allow us to modify the tree for example, swaping two nodes, dimensions, subtrees, or 
adding values and etc. One larger function can generate a postfix expression for how to cut/assemble the modules. We are able to make the function
using recursion of the left then right subtrees.
*/

// Global variables. The global variables will be effective after the input has been parsed
...
int is_leaf_node(node_t* ptr) {
  if(ptr->right == NULL && ptr->left == NULL){				// If it does not have any children, it is a leaf node
    return 1;
  }
  return 0;
}
...
int is_internal_node(node_t* ptr) {
  if(is_leaf_node(ptr)==0) {						// If it is not a leaf node, it must be an internal node
    return 1;
  }
  return 0;
}
...
int is_in_subtree(node_t* a, node_t* b) {
  if(a==NULL) {								// If a is not a node in the tree, return 0
    return 0;
  }
  if(a==b) {								// If a = b, b is a part of the subtree
    return 1;
  }
  if(is_in_subtree(a->left, b) == 1 || is_in_subtree(a->right,b) == 1) { 
    									// Recursively call function with left and right children
    return 1;
  }
  return 0;
}
...
void rotate(node_t* ptr) {						// basic swap function
  int temp = ptr -> module ->h;						//temp to hold height
  ptr->module->h = ptr->module->w;					// height = width
  ptr->module->w = temp;						// width = temp which was the height
}
...
void recut(node_t* ptr) {
  ...
  assert(ptr->module == NULL && ptr->cutline != UNDEFINED_CUTLINE);	//Cheking if ptr is valid to 'recut'
  if(ptr->cutline==V) {							// If vertical cutline assigned, make it horizontal
    ptr->cutline=H;
  }
  else {
    ptr->cutline=V;			 				// else, make it vertical
  }
  return;
}
...
void swap_module(node_t* a, node_t* b) {
  ...
  assert(a->module != NULL && a->cutline == UNDEFINED_CUTLINE);		//Checking if nodes are valid to swap
  assert(b->module != NULL && b->cutline == UNDEFINED_CUTLINE);

  module_t *temp = a->module;						// basic swap function implementation with a temp variable
  a->module = b->module;
  b->module = temp;
}
...
void swap_topology(node_t* a, node_t* b) {
    if(a == NULL || b == NULL) return;			//Subsequent four lines make sure a/b are valid to swap topologies
    if(a->parent == NULL || b->parent == NULL) return;
    if(is_in_subtree(a, b) || is_in_subtree(b, a)) return;
    assert(a->parent != NULL && b->parent != NULL);
    //Since a is getting swapped we need to keep it and b's parents in temporary variables to swap multiple times
    node_t* temp = a;    
    node_t* temp2 = b->parent;

    //There are a few combinations of sitations a and b could be in 
    //Based on these sitations we need to swap parents propely  
     if (a->parent->left == a) {  		//If a is on the left side, swap with b and update b's parent
       a->parent->left = b;
       b->parent = a->parent;
     }
     else if (a->parent->right == a) {  	//If a in on the right, swap with b and update b's parent
       a->parent->right = b;
       b->parent = a->parent;
     }

     if (temp2->left == b) {     		//Since b is getting swapped we need to use temp if B is on the left
         temp2->left = temp;
       temp->parent = temp2;
     }
    
    
     else if (temp2->right == b) { 		//If b is on the right of temp2, swap and update temp's parent
       temp2->right = temp;
       temp->parent = temp2;
     }
...
void postfix_traversal(node_t* ptr, int* nth, expression_unit_t* expression) {
  if(ptr == NULL) return;
    postfix_traversal(ptr->left, nth, expression);	   //Recursively calling postfix_traversal with left then right child
    postfix_traversal(ptr->right,nth, expression);
  if(ptr->module != NULL){
     expression[*nth].module = ptr->module;		   //If module exists, set the nth module in expression
  }else if(ptr->cutline != UNDEFINED_CUTLINE) {		   //If cutline exists, set the nth cutline in expression
...
{
  int sum = 0;
  for(int i=0; i<num_modules; i++) {            	//Traverse through all modules by using global variable  num_modules
     sum += modules[i].resource;   			// Adding the resource at each module to the 'sum'
}
  return sum;
}
...
node_t* init_slicing_tree(node_t* par, int n) {
  ...
  assert(n >= 0 && n < num_modules);
    ...
    //Creating new node and initializing members
    node_t* CurrentNode = (node_t*)malloc(sizeof(node_t));			

    CurrentNode -> module = NULL;
    CurrentNode->cutline = V;
    CurrentNode -> parent =par;
    //This is the base case once your n value is equal to the num of values you have  to return the node since all of them have been accounted for
    //If at last module, update CurrentNode members accordingly 
    if (n == num_modules - 1) {
        CurrentNode->module = &modules[n];
        CurrentNode->cutline = UNDEFINED_CUTLINE;
        CurrentNode->parent = par;
    ...
    //Creating new leaf node and initializing members
    node_t* bruh = (node_t*)malloc(sizeof(node_t));
    bruh->module = &modules[n];
    bruh->cutline = UNDEFINED_CUTLINE;
    bruh->parent = CurrentNode;
    bruh->left = NULL;
    bruh->right = NULL;

    //Recusivly call init_slicing with n+1 so eventially it will reach the base case    
    node_t* leftNode = init_slicing_tree(CurrentNode, n+1);
    //Once you reach the base case this code will run for all previous calls -> set the left and right nodes of each call
    CurrentNode ->left = leftNode;
    CurrentNode ->right = bruh ;
